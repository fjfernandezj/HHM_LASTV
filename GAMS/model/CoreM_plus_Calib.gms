...
*   ---- LES parameters
$gdxin basedata\lespar_0208.gdx
$load  lespar
$gdxin
...
eq_AgrInc_LP(h)..         Z(h) =e= sum(j, [sldq(h,j)+ cs(h,j)]*prcg(h,j)) + sb(h)
                          - sum((a,s)$map_has(h,a,s), acst(h,a,s)*x(h,a,s)) - sum(j,bght(h,j)*prcg(h,j)) -sum(ls, LabWage*HLAB(h,ls));
...
eq_cshcnstrnt_LP(h)..      sum(j, sldq(h,j)*prcg(h,j)) + sb(h)+ exinc(h) + sum(ls, LABEARN(h,ls))
                        =g= sum(j, bght(h,j)*prcg(h,j)) +  sum((a,s)$map_has(h,a,s), acst(h,a,s)*x(h,a,s)) + sum(ls, LabWage*HLAB(h,ls)) ;
...
* Gross Margin
REPORT7(h,a,s,'pmpModel') = [x.l(h,a,s)*yld(h,a,s)*pprice(a)] - [ALPHACST(h,a,s)*x.l(h,a,s)** BETACST(h,a,s)*x.l(h,a,s)]  ;
...
REPORT7(h,a,s,'YdChg') = [sum(j, x.l(h,a,s)*yl(h,a,s,j)*pprice(a))] - [ALPHACST(h,a,s)*x.l(h,a,s)** BETACST(h,a,s)*x.l(h,a,s)]  ;
...
REPORT7(h,a,s,'Yd_lessW') = [sum(j, x.l(h,a,s)*yl(h,a,s,j)*pprice(a))] - [ALPHACST(h,a,s)*x.l(h,a,s)** BETACST(h,a,s)*x.l(h,a,s)]  ;
...
REPORT7(h,a,s,'Yd_IrrEff') = [sum(j, x.l(h,a,s)*yl(h,a,s,j)*pprice(a))] - [ALPHACST(h,a,s)*x.l(h,a,s)** BETACST(h,a,s)*x.l(h,a,s)]  ;
...
Option  REPORT0:1
        ...
        REPORT3:1
;
Display chPMP, chIrrland, chTtlLnd, REPORT0, REPORT1, REPORT2, REPORT3, REPORT4, REPORT5, REPORT6, REPORT7;
